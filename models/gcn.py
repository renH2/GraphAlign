...
class GCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, nlayers=2, dropout=0.5, lr=0.01, weight_decay=5e-4,
                 with_relu=True, with_bias=True, with_bn=False, device=None, plot=False):
    ...
    def fit_with_train(self, features, adj, labels, data, train_iters=200, initialize=True, verbose=False,
                       normalize=True, patience=None, noval=False, **kwargs):
        '''data: full data class'''
        if initialize:
            self.initialize()
        ...
        if noval:
            self._train_with_train(labels, data, train_iters, verbose, adj_val=True)
        else:
            self._train_with_val(labels, data, train_iters, verbose)

    def fit(self, features, adj, labels, data, args, train_iters=200, regularization=None, initialize=True,
            verbose=False,
            normalize=True, patience=None, noval=False, **kwargs):
        '''data: full data class'''
        if initialize:
        ...
        feat_full, adj_full = utils.to_tensor(data.feat_train, data.adj_train, device=self.device)
        # feat_full = row_normalize_tensor(feat_full - feat_full.min())
        ...
        for i in range(train_iters):
            ...
            if regularization == "mmd":
                output_test = self.predict_test(data.feat_test, data.adj_test)
                mmd_loss = MMD(output, output_test)
                loss_train = loss_train + mmd_loss * args.reg
            elif regularization == "cmd":
                output_test = self.predict_test(data.feat_test, data.adj_test)
                cmd_loss = CMD(output, output_test)
                loss_train = loss_train + cmd_loss * args.reg
            loss_train.backward()
            optimizer.step()
            ...
            with torch.no_grad():
                ...
                train_acc = utils.accuracy(output, labels_train)

                # output_test = self.predict_test(data.feat_test, data.adj_test)
                # test_acc = utils.accuracy(output_test, labels_test)

                loss_list.append(loss_train.item())
                train_acc_list.append(train_acc.item())
                # test_acc_list.append(test_acc.item())

                if train_acc > best_acc_val:
                    best_acc_val = train_acc
                    self.output = output
        ...
        # if tsne is True:
        #     fig, ax = plt.subplots()
        #     label_to_color = {0: 'red', 1: 'blue', 2: 'green', 3: 'purple', 4: 'orange'}
        #     # node_colors = labels_test.cpu().numpy()
        #     node_labels = labels_test.cpu().numpy()
        #     num_classes = len(set(node_labels))
        #
        #     # nn_color = [label_to_color[node_type] for node_type in node_labels]
        #     # plt.axis('off')
        #     t_sne_embeddings = TSNE(n_components=2, perplexity=30, method='barnes_hut').fit_transform(
        #         output_test.cpu().numpy())
        #
        #     ax.set_xticklabels([])
        #     ax.set_yticklabels([])
        #
        #     for class_id in range(num_classes):
        #         ax.scatter(t_sne_embeddings[node_labels == class_id, 0], t_sne_embeddings[node_labels == class_id, 1],
        #                    s=20,
        #                    color=label_to_color[class_id], alpha=0.8, linewidths=0.2)
        #     plt.savefig("tsne-our1.pdf", dpi=1000)
        #     # plt.show()
    ...
    def _train(self, labels, data, train_iters, verbose, adj_val=False):
        if adj_val:
        ...
        for i in range(train_iters):
            ...
            with torch.no_grad():
                ...
                if adj_val:
                    loss_val = F.nll_loss(output, labels_train)
                    acc_val = utils.accuracy(output, labels_train)
                else:
                    loss_val = F.nll_loss(output[data.idx_val], labels_train)
                    acc_val = utils.accuracy(output[data.idx_val], labels_train)
    ...
    def _train_with_train(self, labels, data, train_iters, verbose, adj_val=False):
        if adj_val:
            feat_full, adj_full = data.feat_train, data.adj_train
        else:
            feat_full, adj_full = data.feat_full, data.adj_full
    ...
    @torch.no_grad()
    def predict_test(self, features=None, adj=None):
        self.eval()
